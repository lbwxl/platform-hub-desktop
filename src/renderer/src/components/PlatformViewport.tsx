...
export interface PlatformViewportProps {
  ...
  onViewportBounds: (bounds: { x: number; y: number; width: number; height: number }) => void
}
...
export function PlatformViewport(props: PlatformViewportProps) {
  ...
  return <section className="platform-viewport">
    <header className="viewport-header"><div><div className="breadcrumb"><span>店铺列表</span><b>/</b><strong>{props.account.label}</strong></div><h1>{props.platform?.label || props.account.platform} 工作台</h1><p>{ready ? 'Hook Runtime 已就绪，可调用消息、商品与订单能力。' : '请直接在当前工作台完成真人登录，登录后能力会自动恢复。'}</p></div><div className={`runtime-status ${ready ? 'ready' : ''}`}><span className="status-light" /><div><strong>{ready ? 'Runtime 就绪' : '等待登录'}</strong><small>{props.status?.message || props.status?.url || '页面会话准备中'}</small></div></div></header>
    <div className="official-workspace">
      <section className="workspace-card platform-frame"><div className="card-title"><div><strong>官方消息工作台</strong><span>当前店铺的 primary WebContents 使用该店铺独立 Electron partition。</span></div><span className="frame-pill"><Wifi size={14} />{ready ? '已连接' : '未登录'}</span></div><div ref={viewportRef} className="platform-frame-body" aria-label={`${props.account.label} 官方消息工作台`} /></section>
    </div>
    <details className="developer-drawer">
      <summary><span>开发者 / 真人验收工具</span><small>默认不占用官方客服工作台空间</small></summary>
