...
export function normalizeGoofishAuth(value: unknown, account?: unknown, checkedAt = Date.now()): HookAuthState {
...
export function normalizeGoofishSession(value: unknown): HookSessionSummary | undefined {
...
export function normalizeGoofishMessage(
  value: unknown,
  context: { conversationId?: string; selfId?: string; sentByHook?: boolean; receivedAt?: number } = {},
  ...
  const conversationId = text(
    item.conversationId ?? item.sessionId ?? item.cid ?? asRow(item.sessionInfo).sessionId ?? context.conversationId,
  ).replace(/@goofish$/i, '')
  ...
  let origin: HookMessageOrigin = 'unknown'
  if (system) origin = 'system'
  else if (context.sentByHook) origin = 'automation'
  else if (explicitOrigin) origin = explicitOrigin
  else if (manualEvidence && direction === 'outbound') origin = 'human'
  else if (direction === 'inbound' && senderId && (!selfId || senderId !== selfId)) origin = 'customer'
...
export function normalizeGoofishProduct(value: unknown, fallbackShopId = ''): HookProduct | undefined {
  const item = asRow(value)
...
function isNumeric(value: string): boolean { return /^\d+$/.test(value) }
...
function typeCode(item: Row, content: Row): string {
  return text(item.messageType ?? item.msgType ?? item.type ?? content.contentType ?? content.type).toLowerCase()
}

function messageType(item: Row, content: Row, system: boolean): HookMessageType {
  if (system) return 'system'
  const code = typeCode(item, content)
  if (['1', 'text', 'txt', 'plain'].includes(code)) return 'text'
  if (['2', 'image', 'picture', 'photo'].includes(code)) return 'image'
  if (['26', 'item', 'product', 'goods'].includes(code)) return 'product'
  if (['system', 'notice', 'notification'].includes(code)) return 'system'
  const pics = array(asRow(content.image).pics)
  if (pics.length || content.image || item.imageUrl) return 'image'
  return code ? 'unknown' : 'text'
}

function isSystemMessage(item: Row, content: Row): boolean {
...
function originValue(value: unknown): HookMessageOrigin | undefined {
  const origin = text(value).toLowerCase()
  if (['customer', 'human', 'automation', 'system', 'unknown'].includes(origin)) return origin as HookMessageOrigin
  return undefined
}

function hasManualEvidence(item: Row): boolean {
...
function attachmentFrom(item: Row, content: Row, type: HookMessageType): HookMessage['attachments'] extends Array<infer T> | undefined ? T | undefined : never {
  ...
  if (!url && !name && !mimeType) return undefined
