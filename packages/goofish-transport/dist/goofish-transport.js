...
export class GoofishTransport {
    ...
    get underlyingAccountId() { return this.clientAccountId; }
    get capabilities() { return GOOFISH_OPERATIONS; }
    async start() {
        if (this.stopped)
            throw new Error('GoofishTransport 已停止');
        if (this.started)
            return;
    ...
    async invoke(operation, input = {}) {
        if (this.stopped || !this.started)
            return fail(hookError('RUNTIME_NOT_READY', 'GoofishTransport 尚未启动或已停止', undefined, true));
        if (!GOOFISH_OPERATIONS.includes(operation))
    ...
    async stop() {
        ...
        if (this.clientListener) {
            if (this.client.off)
                this.client.off('event', this.clientListener);
            else
    ...
    async invokeOperation(operation, input) {
        if (operation === 'auth.state')
            return ok(await this.readAuth());
        if (operation === 'messages.listen') {
            const auth = await this.readAuth();
            if (!auth.authenticated)
                return fail(hookError('LOGIN_REQUIRED', '请在闲鱼官方页面完成登录'));
            this.listening = true;
        ...
        if (operation === 'sessions.list') {
            ...
            return ok(rows.map(normalizeGoofishSession).filter((item) => Boolean(item)));
        }
        ...
        if (operation === 'products.list' || operation === 'products.detail') {
            ...
            if (!id)
                return fail(hookError('INVALID_INPUT', '商品 id 必填'));
    ...
    async readAuth() {
        ...
        if (actualId && actualId !== this.clientAccountId)
            this.clientAccountId = actualId;
        return auth;
    }
    ...
    async sendImage(input) {
        ...
        if (!mimeType.toLowerCase().startsWith('image/'))
            return fail(hookError('NOT_SUPPORTED', '闲鱼 Hook 当前只支持图片发送'));
        const source = text(input.dataUrl ?? input.data ?? input.url);
        if (!source)
            return fail(hookError('INVALID_INPUT', '图片 data 必填'));
    ...
    onClientEvent(event) {
        ...
        if (event.eventType === 'account-migrated') {
            ...
            if (previousId === this.clientAccountId && nextId)
                this.clientAccountId = nextId;
            const account = asRow(payload.account);
            if (this.clientAccountId === nextId)
                this.emitAuthChanged(normalizeGoofishAuth({
        ...
        if (event.eventType === 'message-added') {
            ...
            if (!preliminary)
                return;
            if (this.listenWatermark !== undefined && preliminary.timestamp < this.listenWatermark)
                return;
            ...
            if (message)
                this.emit({ type: 'message.created', payload: { message }, timestamp: receivedAt, id: `${this.accountId}:${message.id}` });
            return;
        }
    ...
    emit(event) {
        for (const listener of [...this.listeners]) {
            ...
            catch { /* isolate transport subscribers */ }
        }
    }
...
function mapError(error) {
    ...
    const source = `${error?.code || ''} ${message}`;
    if (/LOGIN_REQUIRED|login required|尚未登录|尚未登录|完成登录|登录已?过期|unauth|未登录/i.test(source))
        return hookError('LOGIN_REQUIRED', message, undefined, true);
    if (/CHALLENGE|captcha|验证码|滑块|安全验证|risk control|风控/i.test(source))
        return hookError('CHALLENGE_REQUIRED', message, undefined, true);
    if (/RATE_LIMITED|频繁|限流|too many requests/i.test(source))
        return hookError('RATE_LIMITED', message, undefined, true);
    if (/timeout|超时/i.test(source))
        return hookError('TIMEOUT', message, undefined, true);
    if (/NOT_SUPPORTED|不支持|未暴露.*接口/i.test(source))
        return hookError('NOT_SUPPORTED', message);
    if (/INVALID_INPUT|缺少.*ID|不能为空|必填/i.test(source))
        return hookError('INVALID_INPUT', message);
    if (/RUNTIME_NOT_READY|正在初始化|页面.*关闭|页面.*加载/i.test(source))
        return hookError('RUNTIME_NOT_READY', message, undefined, true);
    return hookError('PLATFORM_ERROR', message, undefined, true);
}
function fingerprint(type, content) { return `${type}:${content.trim()}`; }
function normalizeConversationId(value) { return value.trim().replace(/@goofish$/i, ''); }
//# sourceMappingURL=goofish-transport.js.map