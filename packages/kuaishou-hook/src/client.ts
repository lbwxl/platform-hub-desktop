import { kuaishouHookScript } from './hook.js'
import type {
  AuthState,
  ChatSession,
  HookEvent,
...
function invocation(method: string, args: unknown[]): string {
  return `(() => {
    ...
    if (!api || typeof api[${JSON.stringify(method)}] !== 'function') {
      throw new Error(${JSON.stringify(`快手小店 Hook 方法不可用: ${method}`)})
    }
    return api[${JSON.stringify(method)}](...${JSON.stringify(args)})
  })()`
}

export function createKuaishouClient(evaluate: CdpEvaluate): KuaishouClient {
  ...
  return {
    install: () => evaluate<void>(kuaishouHookScript),
    getAuthState,
    async waitForLogin(options = {}) {
      ...
      if (!state.authenticated) throw new Error('等待快手小店登录超时，请完成登录后重试')
      return state
    },
    collectProducts: () => invoke<ProductRecord[]>('collectProducts'),
    getProductDetail: (goodsId) => invoke<ProductRecord | OperationResult>('getProductDetail', goodsId),
