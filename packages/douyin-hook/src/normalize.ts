...
export function normalizeDouyinProductStatus(value: unknown): HookProductStatus {
  ...
  if (!status && !statusText) return 'unknown'
  if (/off[_ -]?sale|off.?line|下架|已下架|停售|审核驳回/.test(source) || ['3', '4'].includes(status)) return 'off_sale'
  if (/on[_ -]?sale|selling|online|在售|售卖中|上架/.test(source) || ['1', '2'].includes(status) || raw.is_online === 1 || raw.is_online === true) return 'on_sale'
  if (/draft|草稿/.test(source)) return 'draft'
  return 'unknown'
}
