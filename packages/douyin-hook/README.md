...
- Orders: the lazy persistent commerce page uses a candidate official notification-runtime binding to discover order identifiers, then queries the same-origin official `/api/order/searchlist?...&search_words=<orderId>` endpoint as the authoritative snapshot. The candidate binding remains unconfirmed by real Electron order lifecycle verification: it must not be described as production-verified or used as current-state truth. `getshopbroadcastv3` and `reach/list` are historical/audit sources only; they may aid order-ID discovery but cannot establish a real-time state change. A bounded five-minute reconciliation remains the current disconnect fallback; the primary page only resolves conversation-scoped order context when needed.
