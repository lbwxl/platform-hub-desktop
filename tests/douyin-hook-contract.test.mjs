...
test('Douyin page runtime handshake, normalized operations, events, handoff and lifecycle', async () => {
  ...
  const automatedEcho = { serverId: 'server-echo-different', conversationId: 'conversation-1', sender: 'seller-1', content: '自动回复', isMine: true, ext: { 'p:check_Send': 'platform-marker' }, createTime: Date.now() + 10 }
  context.window.ss._frontStore.conversationsInfo.messagesByConversationId.get('conversation-1').sortedMessages.push(automatedEcho)
  callbacks[0](automatedEcho)
  let events = await runtime.drainEvents()
  assert.equal(events.find((event) => event.type === 'message.created').payload.message.origin, 'automation')
  assert.equal((await runtime.invoke('messages.history', { conversationId: 'conversation-1' })).data.find((message) => message.id === 'server-echo-different').origin, 'automation')
  ...
  callbacks[0]({ serverId: 'manual', conversationId: 'conversation-1', sender: 'seller-1', content: '人工回复', isMine: true, ext: { source: 'pc-web', sender_role: '2', 'p:check_Send': 'manual-send-marker' }, createTime: now + 1 })
